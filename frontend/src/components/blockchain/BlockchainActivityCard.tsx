...
export function BlockchainActivityCard() {
  ...
  const [transactions, setTransactions] = useState<TxActivity[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchTxs = async () => {
      try {
        const sigs = await connection.getSignaturesForAddress(new PublicKey(PROGRAM_ID_STR), { limit: 10 });
        const formattedTxs: TxActivity[] = sigs.map((sig) => {
           const date = sig.blockTime ? new Date(sig.blockTime * 1000) : new Date();
           // Por ahora tratamos todas las transacciones del programa como "mint" porque es el contrato interactuado
           return {
             id: sig.signature,
             type: "mint",
             amount: "?",
          ...
          <div className="max-h-[320px] overflow-y-auto custom-scrollbar">
            <AnimatePresence>
              {transactions.length === 0 ? (
                 <div className="p-4 text-center text-sm text-[#6b6b6b]">No recent transactions found</div>
              ) : (
